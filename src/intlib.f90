...
! 
  ...
  if ( 6.0D+00 * abs ( theta ) <= 1.0D+00 ) then
    ...
    gamm = 4.0D+00 * ( sint - theta * cost ) / theta**3
...
!*****************************************************************************80
!
!! r8_gamma calculates the Gamma function for a real argument X.
!
!  Definition:
!
...
!
!    Output, real ( kind = 8 ) R8_GAMMA, the value of the function.  The program
...
!
  if ( y <= 0.0D+00 ) then
    ...
    if ( r8_gamma /= 0.0D+00 ) then

      if ( y1 /= aint ( y1 * 0.5D+00 ) * 2.0D+00 ) then
        parity = .true.
      end if

      fact = - PI / sin ( PI * r8_gamma )
      y = y + 1.0D+00

    else

      r8_gamma = huge ( r8_gamma )
      return
...
!
    ...
    r8_gamma = xnum / xden + 1.0D+00
!
!  Adjust result for case  0.0D+00 < argument < 1.0.
!
    if ( y1 < y ) then
      r8_gamma = r8_gamma / y1
!
