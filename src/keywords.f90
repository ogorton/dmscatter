module keywords
  ...
  subroutine readcontrolfile()
    implicit none
    character(100) :: line, keyvaluestr
    ...
    do while(.true.)
      ...
      read(controlfileid,*,err=110)keyword, keyvaluestr
      if (keyword == coefkey) then
        ...
        call addKeywordpair(keyword, keyvaluestr)
        call setkeyword(keyword,keyvaluestr)
      endif
  ...
  subroutine setkeyword(keyword, keyvaluestr)
    ...
    case('outfile')
      outfile = trim(keyvaluestr)
      print*,trim(keyword),': Set output filename to ',trim(keyvaluestr)
