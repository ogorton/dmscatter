...
iOperatorsdNa[46]={sdNa31x2521=-0.0570678,3,1,2,5,2,1};
...
sdgXe129[iOperator26]={sdgXe10x4545=0.1387504,1,0,4,5,4,5};
...
sdgXe132[16]={sdgXiOperatore00x511511=8.1643047,0,0,5,11,5,11};
...
MJE[y_,{ncapp_,jp_},{ncap_,iOperatorj_},jcap_]:=mjelement[
	y,
	{Nodal[ncapp,jp],Lnumber[ncapp,jp],jp},
	{Nodal[ncap,j],Lnumber[ncap,j],j},
	jcap]

MJ[y_,{ncapp_,jp_},{ncap_,j_},jcap_]:=If[
	PhysicalConditionsNormal[ncapp,jp,ncap,j,jcap],
...
(* This FF function is for when the same operator is used twice *)
FF[DMmatrix_,Operator_,J_,T_]:=Block[
	{Response},
	If[
		Operator==MJ&&UseHelm==True,
		Return[FHelm={{ZZ^2,ZZ(M-ZZ)},{ZZ(M-ZZ),(M-ZZ)^2}}*(3SphericalBesselJ[1,2y^(1/2) r0Helm/b]/(2y^(1/2) r0Helm/b))^2*Exp[-4y s^2/b^2]];
	];

	Response[iDMmatrix_,iOperator_,iT_]:=Response[iDMmatrix,iOperator,iT]=Simplify[
		Block[
			{},
			Sum[ 
				iDMmatrix[i][[1]] (
...
(* This FF function is for when there are two unique operators *)
FF[DMmatrix_,Operator1_,Operator2_,J_,T_]:=Block[
	{Response},
	Response[iDMmatrix_,iOperator_,iT_]:=Response[iDMmatrix,iOperator,iT]=Simplify[
		Block[
			{},
...
ResponseNuclear[yyy_,jjj_,tau1_,tau2_]:=Block[
	{Operator1, Operator2},
	Operator1=0;
	Operator2=0;
	If[
		jjj<1||jjj>8||(!(jjj\[Element]Integers)), 
		Print["Error: Operator index must be an integer between 1 to 8, corresponding to the eight response functions in Eqn. 39 of documentation."];
		Return[];
	];
	If[
		!((tau1===0||tau1===1)&&(tau2===0||tau2===1)), 
		Print["Error: isospin indices must be either 0 or 1."];
		Return[];
	];
	...
	If[1<=jjj<=6,
		If[ 
			tau1==0&&tau2==0,
			Return[E^(-2y)*(2JIso+1)/(4Pi)*1/4(
				FF[DensityMatrix,Operator1,JIso,TIso][[2,2]]
				+FF[DensityMatrix,Operator1,JIso,TIso][[1,1]]
				+FF[DensityMatrix,Operator1,JIso,TIso][[1,2]]
				+FF[DensityMatrix,Operator1,JIso,TIso][[2,1]]
			)/.y->yyy/.FormalReplace//Simplify//MyChop]
		];
		If[
			tau1==1&&tau2==1,
			Return[E^(-2y)*(2JIso+1)/(4Pi)*1/4(
	...
	If[7<=jjj<=8,
		If[
			...
			Return[E^(-2y)*(2JIso+1)/(4Pi)*1/4(
				...
				+FF[DensityMatrix,Operator1,Operator2,JIso,TIso][[2,1]]
			)/.y->yyy/.FormalReplace//Simplify//MyChop]
		];
		If[
			tau1==1&&tau2==1,
...
Fv0MBcutoff[\[Eta]_,xmin_,xesc_]:=
	(
		-4 \[Eta] (-3-3 xesc^2+3 xmin^2+\[Eta]^2)
...
FvsqMBcutoff[\[Eta]_,xmin_,xesc_]:=
	(
		-(
			E^(-2 (xmin^2+\[Eta]^2)) (
				-30 E^(xesc^2+(xmin-\[Eta])^2) (xmin-\[Eta])
				+30 E^(xesc^2+(xmin+\[Eta])^2) (xmin+\[Eta])
				+ 4 E^(2 (xmin^2+\[Eta]^2)) \[Eta] (
					-15 (2+2 xesc^2+xesc^4-xmin^4)
