...
Response[iDMmatrix_,iOperator_,iT_]:=Response[iDMmatrix,iOperator,iT]=Simplify[Block[{},
    Sum[iDMmatrix[i][[1]](If[iDMmatrix[i][[3]]==0,
                             Sqrt[2]/Sqrt[2iT+1] ((an+ap)/2),
                             -(Sqrt[6iT]/Sqrt[(2iT+1)(iT+1)])((ap-an)/2)])
        *E^y (iOperator[y,{iDMmatrix[i][[4]],iDMmatrix[i][[5]]/2},{iDMmatrix[i][[6]],iDMmatrix[i][[7]]/2},iDMmatrix[i][[2]]])
        Jbase[iDMmatrix[i][[2]]],
        {i,DMmatrixLength}]]];
Simplify[(4\[Pi])/(2J+1) Sum[
    1/2 Table[D[(
        Coefficient[
            Response[DMmatrix,Operator,T],
            Jbase[n]]^2
        ),
        {ap,an}[[ii]],{ap,an}[[jj]]],
    {ii,2},{jj,2}],
...
Response[iDMmatrix_,iOperator_,iT_]:=Response[iDMmatrix,iOperator,iT]=Simplify[Block[{},
    (* OCG: iDMmatrix[i][[3]] is isospin Tt *)
    Sum[iDMmatrix[i][[1]](If[iDMmatrix[i][[3]]==0,
                             Sqrt[2]/Sqrt[2iT+1] ((an+ap)/2),
                             -(Sqrt[6iT]/Sqrt[(2iT+1)(iT+1)])((ap-an)/2)])
        *E^y (iOperator[y,
                    {iDMmatrix[i][[4]],iDMmatrix[i][[5]]/2},
                    {iDMmatrix[i][[6]],iDMmatrix[i][[7]]/2},
                    iDMmatrix[i][[2]]])
        Jbase[iDMmatrix[i][[2]]],
        {i,DMmatrixLength}]]];
...
Print["ff"]
Print["a an1 + b ap1"]
Print[Simplify[E^(-2y)Response[DMmatrix,Operator1,T]/.{y->2.2189212177173530*10^(-005)}]];
Print["c an2 + d ap2"]
Print[Simplify[E^(-2y)Response[DMmatrix,Operator2,T]/.{y->2.2189212177173530*10^(-005)}]];
*)

(*OCG: 
    This block takes the two reponse functions above for the two operators 1 and 2:
        (a an1 + b ap1)     (c an2 + d ap2)
    and extracts the proton-neutron form factors using <facepalm>... the derivative
    operator:
        bd  bc
        ad  ac
*)
Simplify[(4\[Pi])/(2J+1)Sum [
    Table[D[(
        Coefficient[
            Response[DMmatrix,Operator1,T]/.{ap->ap1,an->an1},
            Jbase[n]]
        )(
        Coefficient[
            Response[DMmatrix,Operator2,T]/.{ap->ap2,an->an2},
            Jbase[n]]
        ),
        {ap1,an1}[[ii]], {ap2,an2}[[jj]]],
    {ii,2},{jj,2}],
{n,0,Jmax}]]
];
...
If[1<=jjj<=6,
If[tau1==0&&tau2==0,Return[E^(-2y)*(2JIso+1)/(4Pi)*1/4(
     FF[DensityMatrix,Operator1,Operator1,JIso,TIso][[2,2]]
    +FF[DensityMatrix,Operator1,Operator1,JIso,TIso][[1,1]]
    +FF[DensityMatrix,Operator1,Operator1,JIso,TIso][[1,2]]
    +FF[DensityMatrix,Operator1,Operator1,JIso,TIso][[2,1]])/.y->yyy/.FormalReplace//Simplify//MyChop]
];
If[tau1==1&&tau2==1,Return[E^(-2y)*(2JIso+1)/(4Pi)*1/4(
     FF[DensityMatrix,Operator1,Operator1,JIso,TIso][[2,2]]
    +FF[DensityMatrix,Operator1,Operator1,JIso,TIso][[1,1]]
    -FF[DensityMatrix,Operator1,Operator1,JIso,TIso][[1,2]]
    -FF[DensityMatrix,Operator1,Operator1,JIso,TIso][[2,1]])/.y->yyy/.FormalReplace//Simplify//MyChop]
];
If[tau1==0&&tau2==1,Return[E^(-2y)*(2JIso+1)/(4Pi)*1/4(
    -FF[DensityMatrix,Operator1,Operator1,JIso,TIso][[2,2]]
    +FF[DensityMatrix,Operator1,Operator1,JIso,TIso][[1,1]]
    -FF[DensityMatrix,Operator1,Operator1,JIso,TIso][[1,2]]
    +FF[DensityMatrix,Operator1,Operator1,JIso,TIso][[2,1]])/.y->yyy/.FormalReplace//Simplify//MyChop]
];
If[tau1==1&&tau2==0,Return[E^(-2y)*(2JIso+1)/(4Pi)*1/4(
    -FF[DensityMatrix,Operator1,Operator1,JIso,TIso][[2,2]]
    +FF[DensityMatrix,Operator1,Operator1,JIso,TIso][[1,1]]
    +FF[DensityMatrix,Operator1,Operator1,JIso,TIso][[1,2]]
    -FF[DensityMatrix,Operator1,Operator1,JIso,TIso][[2,1]])/.y->yyy/.FormalReplace//Simplify//MyChop]
...
(* This printed quantity is equal to my proton-neutron form factors, up to a
phase, which is flipped. 2022.1.6. *)
(*
Print["{ {bd, bc}, {ad, ac} }"];
Print[(2JIso+1)/(4Pi)FF[DensityMatrix,Operator1,Operator2,JIso,TIso]/.{y->yyy}];    
*)
If[tau1==0&&tau2==0,Return[E^(-2y)*(2JIso+1)/(4Pi)
    *1/4(FF[DensityMatrix,Operator1,Operator2,JIso,TIso][[2,2]]
    +FF[DensityMatrix,Operator1,Operator2,JIso,TIso][[1,1]]
    +FF[DensityMatrix,Operator1,Operator2,JIso,TIso][[1,2]]
    +FF[DensityMatrix,Operator1,Operator2,JIso,TIso][[2,1]])/.y->yyy/.FormalReplace//Simplify//MyChop]
];
If[tau1==1&&tau2==1,Return[E^(-2y)*(2JIso+1)/(4Pi)
    *1/4(FF[DensityMatrix,Operator1,Operator2,JIso,TIso][[2,2]]
    +FF[DensityMatrix,Operator1,Operator2,JIso,TIso][[1,1]]
    -FF[DensityMatrix,Operator1,Operator2,JIso,TIso][[1,2]]
    -FF[DensityMatrix,Operator1,Operator2,JIso,TIso][[2,1]])/.y->yyy/.FormalReplace//Simplify//MyChop]
...
If[tau1==0&&tau2==1,Return[E^(-2y)*(2JIso+1)/(4Pi)*1/4(
    -FF[DensityMatrix,Operator1,Operator2,JIso,TIso][[2,2]]
    +FF[DensityMatrix,Operator1,Operator2,JIso,TIso][[1,1]]
    -FF[DensityMatrix,Operator1,Operator2,JIso,TIso][[1,2]]
    +FF[DensityMatrix,Operator1,Operator2,JIso,TIso][[2,1]])/.y->yyy/.FormalReplace//Simplify//MyChop]
];
...
(* OCG:
    The UnitStep[x] function is 0 for x<0 and 1 for x>=0.
    UnitStep[x1,x2,...] is 1 only if none of the xi are negative.
    eta = vve/vv0
    xmin = q/(2mu)
    We see below essentially evaluating the step function at vesc+vve.
    Implies require: vesc > vmin - ve 
*)
Fv0MBcutoff[\[Eta]_,xmin_,xesc_]:=(-4 \[Eta] (-3-3 xesc^2+3 xmin^2+\[Eta]^2)
        +3 E^xesc^2 Sqrt[\[Pi]] (Erf[xmin-\[Eta]]-Erf[xmin+\[Eta]])
    )/(2 \[Eta] (6 xesc+4 xesc^3-3 E^xesc^2 Sqrt[\[Pi]] Erf[xesc])) 
    UnitStep[xesc-xmin-\[Eta]]
    + ( 2 (xesc-xmin+\[Eta]) (3+(2 xesc+xmin-\[Eta]) (xesc-xmin+\[Eta]))
        +3 E^xesc^2 Sqrt[\[Pi]] (-Erf[xesc]+Erf[xmin-\[Eta]])
    )/(2 \[Eta] (6 xesc+4 xesc^3-3 E^xesc^2 Sqrt[\[Pi]] Erf[xesc])) 
    UnitStep[xesc-xmin+\[Eta],-xesc+xmin+\[Eta]];

FvsqMBcutoff[\[Eta]_,xmin_,xesc_]:=(-(E^(-2 (xmin^2+\[Eta]^2)) (-30 E^(xesc^2+(xmin-\[Eta])^2) (xmin-\[Eta])
        +30 E^(xesc^2+(xmin+\[Eta])^2) (xmin+\[Eta])
        +4 E^(2 (xmin^2+\[Eta]^2)) \[Eta] (-15 (2+2 xesc^2+xesc^4-xmin^4)
        -10 (1+xesc^2) \[Eta]^2+\[Eta]^4)
        +15 E^(xesc^2+2 (xmin^2+\[Eta]^2)) Sqrt[\[Pi]] (1+2 \[Eta]^2) (-Erf[xmin-\[Eta]]+Erf[xmin+\[Eta]])
    ))/(20 \[Eta] (6 xesc+4 xesc^3-3 E^xesc^2 Sqrt[\[Pi]] Erf[xesc])))
    UnitStep[xesc-xmin-\[Eta]]
    +((E^(-xmin^2-\[Eta]^2) (2 E^(xmin^2+\[Eta]^2) (15 xesc
        +10 xesc^3
        +4 xesc^5
        -10 xmin^3
        -10 xesc^2 xmin^3
        +6 xmin^5
        +15 (2+2 xesc^2+xesc^4-xmin^4) \[Eta]
        +10 (3 xesc+2 xesc^3+xmin^3) \[Eta]^2
        +10 (1+xesc^2) \[Eta]^3-\[Eta]^5)
    +15 E^xesc^2 (-2 E^(2 xmin \[Eta]) (xmin+\[Eta])
        +E^(xmin^2+\[Eta]^2) Sqrt[\[Pi]] (1+2 \[Eta]^2) (-Erf[xesc]+Erf[xmin-\[Eta]])
    )))/(20 \[Eta] (6 xesc+4 xesc^3-3 E^xesc^2 Sqrt[\[Pi]] Erf[xesc])))
    UnitStep[xesc-xmin+\[Eta],-xesc+xmin+\[Eta]];
...
If[HALO=="MB",ERTemp= Coefficient[FFTemp,v,0] Fv0MB[vve/vv0,vmin/vv0]/vv0
                     +Coefficient[FFTemp,v,2]FvsqMB[vve/vv0,vmin/vv0]*vv0];
If[HALO=="MBcutoff",ERTemp= Coefficient[FFTemp,v,0] Fv0MBcutoff[vve/vv0,vmin/vv0,vvesc/vv0]/vv0
                           +Coefficient[FFTemp,v,2]FvsqMBcutoff[vve/vv0,vmin/vv0,vvesc/vv0]*vv0];
(*
