...
#plt.clf()
    for dresfile in dresfiles:
        ...
        if (first):
            ...
            R0 = R
        plt.figure(1)
        plt.plot(E,R,label=operatorsymbols[operator-1])
#        plt.figure(2)
#        plt.plot(E,abs(R - R0)/R0,label=operatorsymbols[opi])
    plt.figure(1) 
#    plt.title("%s Xe-131, 500 GeV WIMP, $c^n_%s=$4.8E-4"%(operatorsymbols[opi],
#        operator)) 
    plt.xlabel('$E_{recoil}$ (keV)')
    plt.ylabel('Events/MeV')
    plt.legend(loc=3, fontsize=8, ncol=3, framealpha=0.0)
    plt.xscale('log')
    plt.ylim(10**(-20),10**5)
    if (not all(R==0)):
        plt.yscale('log')
